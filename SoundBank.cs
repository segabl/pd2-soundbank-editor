...
namespace PD2SoundBankEditor {
	...
	public class SoundBank {
		...
		public SoundBank(string file) {
			...
			didxMagicNum = reader.ReadUInt32();
			didxChunkLen = reader.ReadUInt32();
			Trace.WriteLine($"DIDX {didxMagicNum} {didxChunkLen}");
			wemFiles = new List<WemFile>();
			for (var i = 0; i < didxChunkLen / 12; i++) {
				var wem = new WemFile {
			...
			for (var i = 0; i < wemFiles.Count; i++) {
				...
				wem.data = reader.ReadBytes(wem.data.Length);
				var nextOffset = i < wemFiles.Count - 1 ? wemFiles[i + 1].dataOffset : dataChunkLen;
				var unknownDataAmount = nextOffset - wem.dataOffset - wem.data.Length;
		...
		public void Save(string file) {
			...
			writer.Write(didxMagicNum);
			writer.Write(wemFiles.Count * 12); // didxChunkLen
			var totalDataSize = 0;
			foreach (var wem in wemFiles) {
				writer.Write(wem.id);
				writer.Write(totalDataSize);
				writer.Write(wem.data.Length);
